...
fun SignupScreen(viewModel: SignupViewModel = hiltViewModel()) {
    ...
    SignupContent(state = state, interaction = viewModel as SignupInteractionListener)
    EventHandler(effects = viewModel.effect) { effect, nav ->
        when (effect) {
            SignupUiEffect.NavigateToHomeScreen -> {
...
private fun SignupContent(
    state: SignupUiState,
    interaction: SignupInteractionListener
) {
    ...
    ColorBackground(color = White) {
        ...
        Column(
            ...
            PrimaryButton(
                ...
                onClick = interaction::onClickCreateUser
            )
        }
    }
}